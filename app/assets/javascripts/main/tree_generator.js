...
$( document ).ready( function(  ) {
    if ( $( '#willow-canvas' ).length ) {
        var tree = new Tree( 'willow-canvas' );
        tree.grow(  );
    }
} );
...
function Limb ( level, generator, parent, width, x, y ) {
    ...
    var limitingAngle = 4 * Math.PI / 9;
    var parentDx = this.parent.dx;
    var parentDy = this.parent.dy;
    var parentAngle = Math.round( Math.atan2( parentDy, parentDx ) );
    var angle = Math.floor( Math.random(  ) * ( ( parentAngle + limitingAngle ) - ( parentAngle - limitingAngle ) + 1 ) ) + ( parentAngle - limitingAngle );
